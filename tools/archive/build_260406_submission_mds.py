...
HEADER = """<!--
  ...
  저장 위치(작업일): 행정심판청구(원본)/260406/
  파일명: yymmdd_ 접두·파일명 접미 _최종 미사용.
  반영(작업일 전 최종): 260405(인천행심위) 정본·260406(중앙행심위) 초안·작업 자료 전수조사.
-->
...
def _strip_260405_names(s: str) -> str:
    return (
        ...
        .replace("`260405/", "`행정심판청구(원본)/260405(인천행심위)/")
...
def build_gab1() -> None:
    ...
    raw = raw.replace(
        "행정심판청구(원본)/260405(인천행심위)/260405_별지제1호_증거자료_목록.md",
        "행정심판청구(원본)/260406/별지제1호_증거자료_목록.md",
    )
    raw = raw.replace(
        "행정심판청구(원본)/260405/260405_별지제1호_증거자료_목록.md",
        "행정심판청구(원본)/260406/별지제1호_증거자료_목록.md",
    )
