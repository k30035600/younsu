# -*- coding: utf-8 -*-
"""일회성: 저장소 문자열 `행정심판청구(제출용)/` → `행정심판청구(제출용)/최종/` 일괄 반영.
   pathlib `... / "행정심판청구(제출용)" / "갑호증"` → `... / "최종"` 삽입.
"""
...
OLD_SLASH = "행정심판청구(제출용)/"
NEW_SLASH = "행정심판청구(제출용)/최종/"
...
def patch_pathlib_segments(s: str) -> str:
    """`... / "행정심판청구(제출용)" / "갑호증"` → 최종 삽입(이미 첫 세그먼트가 최종이면 유지)."""
    pos = 0
    chunks: list[str] = []
    while True:
        j = s.find(PY_KEY, pos)
        ...
        if after.startswith('최종"'):
            ...
            pos = end
            continue
        chunks.append('"행정심판청구(제출용)" / "최종" / "')
        pos = j + len(PY_KEY)
    return "".join(chunks)


def patch_trailing_pathlib(s: str) -> str:
    """`... / "행정심판청구(제출용)")` 줄 끝 — 루트만 지정한 경우 `최종` 추가."""

    def repl(m: re.Match[str]) -> str:
        return f'{m.group(1)}"행정심판청구(제출용)" / "최종"{m.group(2)}'
