...
**출력 위치:** 국가법령정보센터 요약 PDF(판례 **재추출본**)는 **`행정심판청구(증거)/최종/법령정보/`** 에 **`사건번호_사건명.pdf`** 로 둔다(갑호증 **제출 대상 아님**; 청구서는 사건번호·law.go.kr 원문 인용). **대조·검증을 마친 기존 파일은 삭제하거나 덮어쓰지 않도록** 재실행 시 `python tools/260402_build_prec_pdfs.py --skip-existing` 를 쓴다. **법제처 행정기본법** Q10·Q18은 **`갑제13호증/갑제13-1호증`·`갑제13-2호증`**(각 1페이지 PDF, `pypdf` 필요: `pip install pypdf`). *(구 출력명: `갑제16·17호증_…` 루트)* **전체 원본 PDF**는 `행정심판최종본/행정기본법_질의응답_사례집(최종).pdf` 에 두고, 과거에 판례모음 루트에 두던 `02_…(최종).pdf` 는 **`기타참고/`** 로 옮긴다. 구 `01_`~`11_` 판례모음 경로는 스크립트가 정리한다. 과거에 둔 `*_요약.pdf` 복제본은 스크립트 실행 시 제거한다. **`old/`** 하위는 스크립트가 읽거나 수정하지 않는다. 참고·중복·잡파일·97누8540·91누5358 등은 **`행정심판청구(증거)/최종/기타참고/`** 한곳에 둔다(스크립트 생성분은 **`사건번호_사건명.pdf`**). 구 `판례모음/기타`는 `merge_legacy_panrye_gita()`로 통합. 루트에 남은 중복은 `tidy_panrye_moum_root()`로 같은 `기타참고/`로 옮긴다.
...
def _law_info_case_path(evt: str, case_name: str) -> Path:
    """`행정심판청구(증거)/최종/법령정보/사건번호_사건명.pdf` — 갑호증 제출 대상 아님."""
    return LAW_INFO_DIR / _case_pdf_filename(evt, case_name)


# 국가법령정보 재추출 판례 PDF는 `행정심판청구(증거)/최종/법령정보/`(갑호증 제외). 법제처 Q10·Q18은 `갑제13호증/13-1·13-2`.
